...
The LSTM `.pth` files contain only the model weights and require the corresponding model architecture and vocabulary used during training for loading and inference.

The trained DistilBERT models are not included in the repository due to their size. 
The model directories are available as ZIP archives on Google Drive: 
- [Balanced DistilBERT model](https://drive.google.com/file/d/1QxInwlWMcg8wiadW2IXblDWyhmGp1ms5/view?usp=drive_link)
- [Imbalanced DistilBERT model](https://drive.google.com/file/d/1iTrJ6P8-c5_20onZJJyTGfg7YPqAPPxx/view?usp=drive_link)
...
- **MLP:** Tests hidden-layer shapes `(50,)`, `(100,)`, `(200,)`, and `(100, 50)`. A single 50-neuron hidden layer performed best.
...
- Nikolić, M, and Zečević, A. [*Mašinsko učenje*](https://ml.matf.bg.ac.rs/readings/ml.pdf). Belgrade, 2019.
- University of Belgrade, Faculty of Mathematics. [*Machine Learning Exercise Materials 2026*](https://github.com/matf-ml/materijali-sa-vezbi-2026). Course exercises and presentations.
- Hong, J., Nam, A., and Cai, A. [*Multi-Class Text Sentiment Analysis*](https://cs229.stanford.edu/proj2019aut/data/assignment_308832_raw/26644050.pdf). Stanford University, 2019.

Only the supplied course references are listed here; student-generated exam notes and study guides are intentionally excluded.

### Documentation

- [scikit-learn User Guide](https://scikit-learn.org/stable/user_guide.html) for TF-IDF, linear classifiers, Naive Bayes, MLP, model selection, and evaluation metrics.
- [NLTK Documentation](https://www.nltk.org/) for tokenization, POS tagging, stop words, and WordNet lemmatization.
- [PyTorch Documentation](https://pytorch.org/docs/stable/index.html) for neural-network construction and training.
- [TorchText Documentation](https://pytorch.org/text/stable/index.html) for vocabulary construction.
- [Hugging Face Transformers Documentation](https://huggingface.co/docs/transformers/index) for DistilBERT tokenization, fine-tuning, and evaluation.
